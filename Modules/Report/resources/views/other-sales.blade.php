...
@section('content')
    ...
    <div class="card mt-5">
        <div class="card-header">
            <h4 class="section_title">{{ __('Other Sales Report (Sales From Outside)') }}</h4>
                    ...
                    <tbody>
                        ...
                        @foreach ($sales as $key => $sale)
                            ...
                            @foreach ($outsideProducts as $pIndex => $detail)
                                <tr>
                                    @if($pIndex == 0)
                                        <td rowspan="{{ $outsideProducts->count() }}">{{ ++$rowNumber }}</td>
                                        <td rowspan="{{ $outsideProducts->count() }}">{{ formatDate($sale->order_date) }}</td>
                                        <td rowspan="{{ $outsideProducts->count() }}">{{ $sale->invoice }}</td>
                                        <td rowspan="{{ $outsideProducts->count() }}">{{ $sale?->customer?->name ?? 'Guest' }}</td>
                                    @endif
                                    <td>
                                    ...
                                    <td>{{ currency($detail->purchase_price) }}</td>
                                    <td>{{ currency($detail->selling_price) }}</td>
                                    <td>{{ currency(($detail->selling_price - $detail->purchase_price) * $detail->quantity) }}</td>
                                    @if($pIndex == 0)
                                        <td rowspan="{{ $outsideProducts->count() }}">{{ $sale->sale_note }}</td>
                                    @endif
                                </tr>
                            @endforeach
                        @endforeach
                        <tr style="background-color: #f8f9fa; font-weight: bold;">
                            ...
                            <td>
                                <b>{{ currency($data['total_purchase']) }}</b>
                            </td>
                            <td>
                                <b>{{ currency($data['total_selling']) }}</b>
                            </td>
                            <td>
                                <b>{{ currency($data['total_profit']) }}</b>
                            </td>
                            <td></td>
                        </tr>
                    </tbody>
