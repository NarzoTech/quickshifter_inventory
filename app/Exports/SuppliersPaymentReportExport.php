...
class SuppliersPaymentReportExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithTitle
{
    private $index;
    public function __construct(private $supplierPayments) {}
    /**
    ...
    public function map($supplierPayment): array
    {
        ...
        return [
            ++$this->index,
            $supplierPayment->purchase_date,
            $supplierPayment->invoice_number,
            $supplierPayment->supplier->name,
            $supplierPayment->total_amount,
            $supplierPayment->paid_amount,
            $supplierPayment->due_amount - $supplierPayment->purchaseReturn->sum('return_amount') + $supplierPayment->purchaseReturn->sum('received_amount'),
