...
class StockExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithTitle
{
    ...
    public function map($product): array
    {
        ...
        return [
            ...
            $product->stockDetails->sum('in_quantity') ?? 0,
            $product->stockDetails->sum('out_quantity') ?? 0,
            $product->stock ?? 0,
            remove_comma($stock) * remove_comma($product->avg_purchase_price) ?? 0,
            remove_comma($stock) * remove_comma($selling_price) ?? 0
        ];
    }
